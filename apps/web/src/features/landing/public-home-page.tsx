...
export default function PublicHomePage({ onAuthenticate }: PublicHomePageProps) {
  ...
  return (
    <main className="public-stage min-h-screen overflow-hidden text-obsidian">
      ...
      <section className="public-hero relative" id="top">
        ...
        <div className="relative z-10 mx-auto grid max-w-[94rem] gap-12 px-5 pb-16 pt-16 sm:px-8 lg:grid-cols-[1.12fr_0.88fr] lg:px-12 lg:pb-24 lg:pt-24">
          <div className="hero-copy">
            ...
            <h1 className="hero-title mt-8 max-w-4xl font-display text-6xl font-semibold leading-[0.88] tracking-[-0.05em] text-white sm:text-7xl lg:text-[6.5rem]">Discover government schemes for your <em className="font-normal text-copper">MSME’s next move.</em></h1>
            <p className="hero-lede mt-7 max-w-2xl text-sm leading-7 text-white/60 sm:text-base">Explore cited Central and State MSME schemes, subsidies, grants and credit programmes—then assess potential eligibility through transparent, deterministic rules.</p>
          ...
          <aside className="hero-opportunity relative" aria-label="Featured official programme"><img alt="Indian MSME manufacturing entrepreneur inspecting a precision component" className="hero-portrait absolute inset-0 size-full object-cover" fetchPriority="high" src="/images/indian-msme-founder-v1.png" /><div className="absolute inset-0 bg-gradient-to-t from-[#090c10] via-[#090c10]/65 to-[#090c10]/15" />
      ...
      <section className="official-source-ribbon" aria-label="Official discovery network" data-reveal><p>Evidence network</p><a href={mySchemeUrl} rel="noreferrer" target="_blank"><img alt="myScheme" height="24" loading="lazy" src="/images/official/myscheme-logo.svg" width="136" /><span>National scheme discovery · MeitY</span></a><a href="https://udyamregistration.gov.in/" rel="noreferrer" target="_blank"><Landmark className="size-5" /><strong>Udyam Registration</strong><span>Official Ministry of MSME portal</span></a><a href="https://www.startupindia.gov.in/content/sih/en/government-schemes.html" rel="noreferrer" target="_blank"><BadgeIndianRupee className="size-5" /><strong>Startup India</strong><span>Government schemes directory</span></a></section>

      <section className="mx-auto max-w-[94rem] px-5 pt-20 sm:px-8 lg:px-12" id="saarthi-ai" aria-labelledby="ai-title"><div className="home-chat-showcase" data-reveal><div className="home-chat-copy"><span className="inline-flex items-center gap-2 text-[0.62rem] font-black tracking-[0.16em] text-copper uppercase"><Bot className="size-4" /> Saarthi AI</span><h2 id="ai-title">Find the signal in India’s scheme landscape.</h2><p>Describe your enterprise in everyday language. Saarthi searches reviewed evidence, returns cited guidance, and helps collect the facts needed for a rule-based eligibility assessment.</p><div className="mt-7 flex flex-wrap justify-center gap-2 lg:justify-start"><span>Discover MSME schemes</span><span>Compare official evidence</span><span>Identify missing facts</span></div><button className="hero-primary mt-8" onClick={() => onAuthenticate("register")} type="button">Ask Saarthi AI <ArrowRight className="size-4" /></button></div><div className="home-chat-demo"><header><BrandMark size="sm" /><div><strong>Saarthi</strong><small>Official-source business guidance</small></div><i /></header><div className="home-chat-thread"><div className="home-chat-question">Which programme could help me start a small manufacturing unit?</div><div className="home-chat-response"><BrandMark size="sm" /><p>PMEGP is one official starting point worth reviewing for a new non-farm micro-enterprise. Current terms and lender assessment still apply.<span className="chat-stream-cursor" /></p></div><a href={officialSchemes.find((scheme) => scheme.id === "pmegp")?.sourceUrl ?? mySchemeUrl} rel="noreferrer" target="_blank"><Landmark className="size-4" /><span><strong>Ministry of MSME · PMEGP</strong><small>Official source</small></span><CheckCircle2 className="ml-auto size-4 text-emerald-600" /></a></div><button className="home-chat-composer" onClick={() => onAuthenticate("register")} type="button"><span>Message Saarthi</span><ArrowRight className="size-4" /></button></div></div></section>

      <section className="mx-auto max-w-[94rem] px-5 py-20 text-center sm:px-8 lg:px-12" aria-labelledby="journey-title" data-reveal>
        ...
        <div className="stagger-grid mt-10 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{journeys.map(({ icon: Icon, label, copy }, index) => <a className="journey-card group" href="#programmes" key={label} style={{ "--stagger": index } as CSSProperties}><span className="journey-number">0{index + 1}</span><Icon className="mt-12 size-6 text-copper" /><h3 className="mt-6 font-display text-2xl font-semibold">{label}</h3><p className="mt-2 text-xs leading-6 text-steel">{copy}</p><ChevronRight className="mt-7 size-4 transition group-hover:translate-x-1" /></a>)}</div>
      </section>

      <section className="mx-auto max-w-[94rem] px-5 pb-24 sm:px-8 lg:px-12" id="programmes" aria-labelledby="programmes-title" data-reveal>
        <div className="text-center"><p className="section-kicker">Government schemes for MSMEs</p><h2 className="section-title mx-auto" id="programmes-title">Explore official paths to fund, build and grow.</h2><p className="section-copy mx-auto">Browse a curated library of Indian MSME schemes, subsidies, grants, credit support and registrations. Every material summary links back to its official authority.</p><div className="mt-7 flex max-w-full justify-start gap-2 overflow-x-auto pb-1 sm:justify-center">{schemeCategories.map((item) => <button aria-pressed={category === item} className={`category-pill shrink-0 rounded-full px-4 py-2.5 text-xs font-bold ${category === item ? "bg-obsidian text-white" : "border border-obsidian/10 bg-white/60 text-steel"}`} key={item} onClick={() => setCategory(item)} type="button">{item}</button>)}</div></div>
        <div className="stagger-grid mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{schemes.map((scheme, index) => <article className={`public-scheme-card ${index === 0 ? "public-scheme-featured" : ""}`} key={scheme.id} style={{ "--stagger": index } as CSSProperties}><div className="flex items-start justify-between gap-4"><span className="text-[0.62rem] font-black tracking-[0.15em] text-copper uppercase">{scheme.category} · {scheme.shortTitle}</span><a aria-label={`Open official source for ${scheme.title}`} className="source-arrow" href={scheme.sourceUrl} rel="noreferrer" target="_blank"><ArrowUpRight className="size-4" /></a></div><div className="mt-auto pt-16"><p className="text-[0.65rem] text-steel">{scheme.authority}</p><h3 className="mt-3 font-display text-3xl font-semibold leading-none">{scheme.title}</h3><p className="mt-4 text-xs leading-6 text-steel">{scheme.summary}</p><p className="mt-6 border-t border-current/10 pt-4 text-[0.65rem] leading-5 opacity-65"><strong>For:</strong> {scheme.audience}</p></div></article>)}</div>
      ...
      <section className="state-section relative overflow-hidden py-24" id="states" aria-labelledby="states-title" data-reveal>
        <img alt="" aria-hidden="true" className="state-medallion" loading="lazy" src="/images/brand/saarthi-chariot-medallion-v1-512.png" />
      ...
      <section className="mx-auto max-w-[94rem] px-5 py-24 sm:px-8 lg:px-12" id="plans" aria-labelledby="plans-title"><div className="text-center"><p className="section-kicker">From MSME scheme discovery to a confident decision</p><h2 className="section-title mx-auto" id="plans-title">Explore government schemes free. Build your evidence-backed fit map.</h2><p className="section-copy mx-auto">Official scheme links remain open to everyone. The guided workspace adds structured matching, deterministic eligibility assessments, comparison and evidence-gap tracking—not a paywall around public information.</p></div><div className="mx-auto mt-12 grid max-w-5xl gap-5 lg:grid-cols-2"><article className="plan-card"><p className="text-[0.65rem] font-black tracking-[0.16em] text-steel uppercase">Explorer · Free</p><p className="mt-4 font-display text-5xl font-semibold">₹0</p><p className="mt-3 text-sm text-steel">Discover official Indian MSME schemes and build your business profile.</p><ul className="plan-list"><li>Curated Central government scheme discovery</li><li>Official sources across 36 jurisdictions</li><li>Secure founder workspace and profile</li></ul><button className="plan-button" onClick={() => onAuthenticate("register")} type="button">Start exploring free</button></article><article className="plan-card plan-card-pro"><span className="absolute right-6 top-6 rounded-full bg-copper px-3 py-1 text-[0.58rem] font-black text-obsidian">COMING SOON</span><p className="text-[0.65rem] font-black tracking-[0.16em] text-copper uppercase">Saarthi Pro · Guided</p><p className="mt-4 font-display text-5xl font-semibold text-white">Scheme fit map</p><p className="mt-3 text-sm text-white/48">Turn your business use case and evidence into a prioritised MSME scheme workflow.</p><ul className="plan-list text-white/62"><li>Business-to-scheme fit signals</li><li>Deterministic eligibility assessments</li><li>Scheme comparisons, evidence gaps and alerts</li></ul><button className="plan-button border-white/15 bg-white text-obsidian" onClick={() => onAuthenticate("register")} type="button">Create a free workspace</button></article></div></section>
